...
const integrationValidation = [
  ...
  body('status').optional().isIn(['enabled', 'disabled', 'disconnected'])
  ...
  body('managedBy.*').optional().isMongoId().withMessage('Invalid employee ID in managedBy'),
  body('allowedRoles').isArray({ min: 1 }).withMessage('At least one allowedRole is required'),
  body('allowedRoles.*').isIn(['admin', 'manager', 'employee']).withMessage('Invalid role in allowedRoles'),
...
router.get(
  '/:_id',
  requiresRole('admin', 'manager', 'employee'),
  param('_id').isMongoId().withMessage('Invalid integration ID'),
  getIntegrationById
);
